<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['credit:level:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['credit:level:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="levelList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="奖惩内容" align="center" prop="rewardPunishmentLevelContent" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="奖惩内容">
          <editor v-model="form.rewardPunishmentLevelContent" :min-height="192"/>
        </el-form-item>
        <el-form-item label="备用列1" prop="beiyong1">
...
<script>
import { listLevel, getLevel, delLevel, addLevel, updateLevel } from "@/api/credit/level";
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        rewardPunishmentLevelContent: null,
        beiyong1: null,
        beiyong2: null,
        beiyong3: null
      },
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.rewardPunishmentLevelId)
    ...
    handleUpdate(row) {
      this.reset();
      const rewardPunishmentLevelId = row.rewardPunishmentLevelId || this.ids
      getLevel(rewardPunishmentLevelId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rewardPunishmentLevelId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const rewardPunishmentLevelIds = row.rewardPunishmentLevelId || this.ids;
      this.$modal.confirm('是否确认删除奖惩级别编号为"' + rewardPunishmentLevelIds + '"的数据项？').then(function() {
        return delLevel(rewardPunishmentLevelIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('credit/level/export', {
        ...this.queryParams
