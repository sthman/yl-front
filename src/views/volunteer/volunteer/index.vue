<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="积分" prop="vStar">
        <el-input
          v-model="queryParams.vStar"
          placeholder="请输入积分"
          clearable
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['volunteer:volunteer:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['volunteer:volunteer:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['volunteer:volunteer:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['volunteer:volunteer:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="volunteerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="积分" align="center" prop="vStar" />
      <el-table-column label="用户状态" align="center" prop="vStatus" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="积分" prop="vStar">
          <el-input v-model="form.vStar" placeholder="请输入积分" />
        </el-form-item>
        <el-form-item label="用户id" prop="userId">
...
<script>
import { listVolunteer, getVolunteer, delVolunteer, addVolunteer, updateVolunteer } from "@/api/volunteer/volunteer";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.vStar)
    ...
    handleUpdate(row) {
      this.reset();
      const vStar = row.vStar || this.ids
      getVolunteer(vStar).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const vStars = row.vStar || this.ids;
      this.$modal.confirm('是否确认删除志愿者编号为"' + vStars + '"的数据项？').then(function() {
        return delVolunteer(vStars);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('volunteer/volunteer/export', {
        ...this.queryParams
